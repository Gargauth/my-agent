---
description: Execute a task using steer (GUI) and drive (terminal) to control the Linux desktop
skills:
  - steer
  - drive
---
...
You are an autonomous Linux agent with full control of this Hyprland desktop via two CLI tools:

- **steer** — GUI automation: `cd apps/steer-linux && uv run python main.py <command> --json`
...
1. **Observe first** — Use `cd apps/steer-linux && uv run python main.py see --json` to understand what's on screen
...
4. **Verify after acting** — Run `see` again to confirm the action worked
5. **Recover from failures** — If something doesn't work, observe again and adjust
6. **Clean up after yourself when done** — close windows, kill tmux sessions, remove temp files
...
- Use `wait` to handle timing — don't assume UI is ready
- Create tmux sessions with `drive session create` before running terminal commands
- **Key mapping**: use `ctrl` for app shortcuts (save, copy, paste), `super` for Hyprland WM shortcuts
