...
```bash
yq -i '.summary = "Opened Firefox, captured screenshot with 42 elements, saved to /tmp/steer/a1b2c3d4.png"' apps/listen/jobs/{{JOB_ID}}.yaml
```
...
- **Clean up processes you started** — use `drive proc list --json` to check for processes you spawned. Kill any you don't need running.
- **Remove temp files** you wrote to `/tmp/` that are no longer needed
- **Leave the desktop as you found it** — close windows you opened
